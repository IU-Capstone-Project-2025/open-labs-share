...
export default function CommentsSection({ contentType, contentId, userId, userName }) {
  ...
  const fetchComments = async (pageNum = 1, resetComments = false) => {
    try {
      ...
      const data = await commentsAPI.getArticleComments(contentId, pageNum);
      // The API returns { comments: [...], pagination: { currentPage, totalPages, totalItems } }
      if (resetComments || pageNum === 1) {
        setComments(data.comments);
      } else {
        setComments(prev => [...prev, ...data.comments]);
      }
      setTotalComments(data.pagination?.totalItems || data.comments.length);
  ...
  const fetchReplies = async (commentId) => {
    try {
      ...
      // setExpandedReplies(prev => new Set([...prev, commentId])); // This line is removed
    } catch (error) {
  ...
  const submitComment = async () => {
    ...
    setSubmitting(true);
    try {
      const response = await commentsAPI.createArticleComment(contentId, {
                  ...
                  <div className="mb-4">
                    {editingComment === comment.id ? (
                      ...
                      <div className="relative">
                        ...
                        {isLong && (
                          <button
                            className="absolute bottom-0 right-0 flex items-center px-2 py-1 rounded-tl z-10 text-msc hover:text-msc-dark border border-msc shadow-lg bg-white/90 dark:bg-gray-900/80"
                        ...
                        return (
                          <div key={reply.id} className="bg-white dark:bg-gray-800 rounded-lg p-4 relative">
                            ...
                            {editingComment === reply.id ? (
                              ...
                              <div className="relative">
                                <ReactMarkdown
                                  className={`text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap prose dark:prose-invert max-w-none ${!expandedReply && isReplyLong ? 'max-h-48 overflow-hidden fade-out-mask' : ''}`}
                                  remarkPlugins={[remarkGfm, remarkMath]}
                                  rehypePlugins={[rehypeKatex, rehypeHighlight]}
                                  components={{
                                    h1: HeadingRenderer(1),
